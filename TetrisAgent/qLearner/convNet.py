...
scores = np.zeros(100)
pieces_played = np.zeros(100)
lines_cleared = np.zeros(100)
while True:
    ...
    for t in count():
        ...
        if done:
            scores[current_episode%100] = current_score 
            pieces_played[current_episode % 100] = num_pieces_played
            lines_cleared[current_episode % 100] = current_lines
            if current_episode % 50 == 0:
                  optimize_model()
            if current_episode % 100 == 0:
                  print("score: ", np.mean(scores), "pp: ", np.mean(pieces_played), 
